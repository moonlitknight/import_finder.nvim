...
M.setup = function(opts)
    ...
    vim.api.nvim_create_user_command('ImportFinder', M.find_imports, {range = true})
    vim.api.nvim_set_keymap('v', ',.fi', ':ImportFinder<CR>', {noremap = true, silent = true})
end

-- Return only the Lazy.nvim plugin spec
return {
    "yourusername/import_finder.nvim",
    config = function()
        require("plugins.import_finder").setup({
            root_dir = '/usr/app',
        })
    end,
    keys = {
        { ",.fi", mode = "v", desc = "Find imports" },
    },
    cmd = { "ImportFinder" },
}